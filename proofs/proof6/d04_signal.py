...
def receive(sock):
    raw, ancillary, flags, _ = sock.recvmsg(MAX_PACKET + 1)
    require(raw and not ancillary and flags & ~getattr(socket, 'MSG_EOR', 0) == 0)
    value = decode(raw)
...
def custody_values(helper, initial_env, argv, descriptors):
    allowed = PUBLIC_ENV | SYSTEM_ENV | ({TOKEN_SLOT} if helper else WRITER_ENV)
...
def custody(helper):
    ...
    result = custody_values(helper, initial, Path('/proc/self/cmdline').read_bytes().split(b'\0')[:-1], descriptors)
...
def _permission_evidence(raw, binding):
    ...
    require(variables['system.github.job']['value'] == 'd04_writer')
    job = message['jobId']
    ...
    return dict(source='github.token/current-worker-job-message/v1', permissions=permissions, job='d04_writer', worker_job_id=job,
...
def serve():
    ...
    permissions = _permission_evidence(_worker_log(), b)
    root = directory(b)
    ...
    preflight = status_request(token, b)
    def deadline(*_):
    ...
    if interrupted:
        ...
        if proc.exists():
            require((proc / 'cmdline').read_bytes().split(b'\0')[:-1] == [
                ...
                b'-I', b'-B', b'proofs/proof6/d04_signal.py', b'serve'])
...
if __name__ == '__main__':
    try:
        require(sys.argv[1:] in (['serve'], ['cleanup']))
        serve() if sys.argv[1:] == ['serve'] else cleanup()
