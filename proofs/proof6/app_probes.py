...
def run_d02_probes(writer):
    ...
    ref = writer._call(token, 'GET', BASE + '/git/ref/heads/proof6-writer-runtime-r3f')
    ...
    child = writer._call(token, 'POST', BASE + '/git/commits', {
        'message': 'Proof6 R3f F10 fixed runtime fast-forward negative probe',
        'tree': _sha(commit['tree']['sha']), 'parents': [frozen]})
    candidate = _sha(child['sha'])
    _require(candidate != frozen)
    try:
        writer._call(token, 'PATCH', BASE + '/git/refs/heads/proof6-writer-runtime-r3f',
