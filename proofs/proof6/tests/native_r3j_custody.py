...
import native_r3j_startup as prior
from r3j_custody_fixtures import BASE, CASES, environment, label
...
class NativeCustody(unittest.TestCase):
    ...
    def wrappers(self, old=False):
        ...
        if old:
            self.env = {k:v.replace('runtime-r3j','runtime-r3i') for k,v in self.env.items()}
        ...
        with patch.object(prior.prior.n, 'LOAD', loader), patch.object(s, 'RUNTIME', 'refs/heads/proof6-writer-runtime-r3i' if old else s.RUNTIME):
