...
from r3j_custody_fixtures import BASE, CASES, TOKEN, environment, label
...
class Custody(unittest.TestCase):
    ...
    def test_signal_AST_only_launch_check_and_fixed_stage_change(self):
        from r3j_scope import identity
        old = ast.parse(identity(subprocess.check_output(['git', '-C', str(ROOT), 'show', BASE+':proofs/proof6/d04_signal.py'])))
