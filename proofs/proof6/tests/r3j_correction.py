...
import r3j_prior
sys.modules['test_r3g']=r3j_prior.module
import test_r3g_correction

if __name__=='__main__':
    path=Path(__file__).with_name('reproduce_r3g_review.py')
    module=types.ModuleType('r3j_review');module.__file__=str(path)
    source=path.read_text(encoding='utf-8').replace('PROOF6_R3G','PROOF6_R3J')
    with patch.object(socket,'socket',side_effect=AssertionError('OFFLINE_ONLY')):
        exec(compile(source,'reproduce_r3g_review.py (R3j schemas)','exec'),module.__dict__)
