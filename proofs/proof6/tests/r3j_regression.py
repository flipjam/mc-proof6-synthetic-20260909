...
import r3j_support as support
sys.modules['support']=support
import yaml
# Replace only the old indentation-based extraction with parsed current D04 job.
def _workflow_script(test):
    flow=yaml.load((support.ROOT/'.github/workflows/proof6-writer.yml').read_bytes(),Loader=yaml.BaseLoader)
    return next(s['run'] for s in flow['jobs']['d04_writer']['steps'] if s.get('id')=='execute')
support.legacy.Corrections.workflow_script=_workflow_script
import regression

original = types.ModuleType('test_r3f')
sys.modules['test_r3f'] = original
raw = Path(__file__).with_name('test_r3f.py').read_text()
raw = raw.replace('proof6-writer-runtime-r3f', 'proof6-writer-runtime-r3j').replace('FRESH_R3F','FRESH_R3J')
exec(compile(raw, 'test_r3f.py (R3j identity adapter)', 'exec'), original.__dict__)
