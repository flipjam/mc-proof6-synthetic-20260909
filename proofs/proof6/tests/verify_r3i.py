...
def verify():
    ...
    for name in identity_only:
        ...
        expected=old.replace(b'r3h',b'r3i').replace(b'R3H',b'R3I').replace(b'R3h',b'R3i')
        if name=='writer.py':expected=expected.replace(b'd754156067cf1aa2318e7b04d7fcf47902eb9846',b'2cb629820e3bd04e7c7edaa2b2364682e4dc5f1b')
    ...
    inventory.update(canonical_rows=72,fresh=72,inherited=0,NOT_APPLICABLE=0,
        ...
        canary_credit=0,R3i_provisioned=False,R3i_frozen=False,R3i_executed=False)
    return inventory
