...
class Scope(unittest.TestCase):
    def setUp(self):
        ...
        self.old=yaml.load(subprocess.check_output(['git','-C',str(ROOT),'show',PARTIAL+':'+self.path]),Loader=yaml.BaseLoader)

    def test_D04_workflow_job_byte_semantics_unchanged(self):
        self.assertEqual(self.old['jobs']['d04_writer'],self.flow['jobs']['d04_writer'])
    ...
    def test_setup_token_delivered_only_to_helper_step(self):
        ...
        self.assertIn('d04_signal.py serve-setup &',deliveries[0]['run'])
        self.assertIn('wait "$native"',deliveries[0]['run'])
        peer=next(x for x in job['steps'] if 'qualify-setup' in x.get('run',''))
        self.assertNotIn('env',peer);self.assertNotIn(s.TOKEN_SLOT,peer['run'])
        self.assertIn('20s',peer['run']);self.assertIn('</dev/null',peer['run'])
    ...
    def test_plan_all_rows_and_campaign_except_S1_unchanged(self):
        ...
        old=json.loads(subprocess.check_output(['git','-C',str(ROOT),'show',PARTIAL+':'+path]))
    ...
    def test_original_safety_modules_byte_identical_to_partial(self):
        for name in ('writer.py','journal.py','outage.py','d03_job_token.py','d03_rejection.py','d04_capability.py',
            ...
            self.assertEqual((ROOT/path).read_bytes(),subprocess.check_output(['git','-C',str(ROOT),'show',PARTIAL+':'+path]),name)
