...
RUNTIME_REF = 'refs/heads/proof6-writer-runtime'
...
def get(path):
    request = urllib.request.Request('https://api.github.com/' + path, headers={
        ...
        'Accept': 'application/vnd.github+json', 'User-Agent': 'proof6-runtime-r2'})
...
def guard(manifest):
    ...
    base = 'repos/' + REPO
    ref = get(base + '/git/ref/heads/proof6-writer-runtime')
    _require(ref['ref'] == RUNTIME_REF and ref['object']['sha'] == os.environ['GITHUB_SHA'])
    rule = get(base + '/rulesets/22727382')
    rule = {k: rule[k] for k in RULE_FIELDS}
    _require(rule == {
        'id': 22727382, 'name': 'proof6-runtime-integrity-r2', 'target': 'branch',
    ...
    _require(policy['id'] == 59579447 and policy['name'] == 'proof6-writer-runtime'
...
def reconcile_prior_runs(manifest):
    ...
    while True:
        ...
        prior.extend(run for run in runs['workflow_runs']
                     ...
                     and run['head_branch'] == 'proof6-writer-runtime'
...
def main():
    ...
    # The only key file is ephemeral, mode 0600, on the GitHub-hosted runner.
    # There are no artifact uploads, shell tracing, or secret-bearing outputs.
    key = os.environ.pop('PROOF6_APP_PRIVATE_KEY')
    _require(bool(key))
    with tempfile.TemporaryDirectory(prefix='proof6-', dir=os.environ['RUNNER_TEMP']) as tmp:
        path = Path(tmp) / 'app.pem'
        fd = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
        with os.fdopen(fd, 'w') as stream:
            stream.write(key)
        del key
        writer = _Writer(frozen_manifest=manifest, private_key_path=str(path),
                         openssl_path='/usr/bin/openssl', runtime_guard=guard)
...
if __name__ == '__main__':
    try:
        ...
        print('PROOF6_SETUP_OR_ADMISSION_BLOCKED', flush=True)
        code = 1
    sys.exit(code)
